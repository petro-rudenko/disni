...
public class OdpReadClient implements RdmaEndpointFactory<OdpReadClient.ReadClientEndpoint> {
	...
	public OdpReadClient(String host, int port, int size, int loop) throws IOException{
		this.group = new RdmaPassiveEndpointGroup<OdpReadClient.ReadClientEndpoint>(1, 10, 4, 40);
	...
	public OdpReadClient.ReadClientEndpoint createEndpoint(RdmaCmId id, boolean serverSide)
			throws IOException {
	...
	private void run() throws Exception {
		...
		OdpReadClient.ReadClientEndpoint endpoint = group.createEndpoint();
		...
		long fileSize = recvBuf.getLong();
		System.out.println("ReadClient, receiving rdma information, addr " + addr + ", fileSize " + fileSize + ", key " + lkey);
		...
		for (int i = 0; i < loop; i++){
			...
			endpoint.pollUntil();
			long randomAddress =
				ThreadLocalRandom.current().nextLong(addr,(addr + fileSize) - size);
			sendWR.getRdma().setRemote_addr(randomAddress);
		}
	...
	public static void main(String[] args) throws Exception {
		...
		OdpReadClient client = new OdpReadClient(cmdLine.getIp(), cmdLine.getPort(), cmdLine.getSize(), cmdLine.getLoop());
		client.run();
	}
